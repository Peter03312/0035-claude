...
硬性校验（违反即导入失败或拒绝求解，绝不出具有误导性的方案）：

- **桥宽 ≥ 周长**：单座桥覆盖整圈，导入报错；画布参数输入也会被拒绝；
- **最小桥心距 < 桥宽**：相邻桥覆盖必然搭接，求解器给参数见证；
- **候选/禁区编号重复**：切桥单将无法区分桥位，`candidates[i].id` / `forbidden[i].id` 重复按错误拒绝；
- **候选桥心环向重合**（1 µm 内）：同样按错误拒绝；
- **轮廓外输入点**：自动吸附到最近轮廓边（坐标写入吸附点，绝不把原始离群坐标写进切桥单），
  偏离 > 0.5 mm 时在该字段产生告警，提示制版工核对；
- **单桥方案**：唯一悬空是整圈净长 `L−w`（恒为正），切桥单标为跨接缝行；
  超过最大悬空时按无解处理，不会显示负数。
...
- 高亮：紫色弧=跨接缝悬空，红色粗段=人工禁区，红色虚线=不可行见证，蓝色粗段=已选桥覆盖；
  红色虚线候选短横=桥宽擦碰禁区（不可选）。所有沿轮廓的高亮都是**开放描边路径（fill:none）**，
  近整圈弧段也不会被填成色块遮挡刀线；接缝不在数组起点时，桥宽高亮严格按桥宽长度绘制。
...
- `single-bridge.json` 单桥方案：整圈净长 314 mm 必须为正、标跨接缝；
- `offset-seam.json` 接缝不在 `contour[0]`（在右边），3 mm 桥的高亮只覆盖 3 mm；
...
`verify` 使用 `mcr.microsoft.com/playwright:v1.49.1-jammy` 镜像（内置与
`@playwright/test` **完全同版本** 1.49.1 的 Chromium 与系统库，升级测试工具时必须同步此标签），
